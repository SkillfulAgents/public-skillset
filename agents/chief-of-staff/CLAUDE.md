...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Slack through the SuperAgent API account `slack` (`api_account:slack`); Notion through the SuperAgent API account `notion` (`api_account:notion`); Linear through the SuperAgent API account `linear` (`api_account:linear`).
