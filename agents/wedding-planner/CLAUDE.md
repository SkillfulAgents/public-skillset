...
description: "Track wedding vendor threads, timeline, and budget in Notion; chase unanswered items and send a weekly decisions digest."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Gmail through the SuperAgent API account `gmail` (`api_account:gmail`); Notion through the SuperAgent API account `notion` (`api_account:notion`).
