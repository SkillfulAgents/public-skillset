...
description: "Mirror the global Notion webinar database into a local calendar; add, update, or remove events for chosen regions and types."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Notion through the SuperAgent API account `notion` (`api_account:notion`); Google Calendar through the SuperAgent API account `googlecalendar` (`api_account:googlecalendar`).
