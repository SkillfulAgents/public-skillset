...
description: "Draft security-questionnaire answers from public docs and past responses; cite evidence and flag questions needing a human."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Google Drive through the SuperAgent API account `googledrive` (`api_account:googledrive`); Notion through the SuperAgent API account `notion` (`api_account:notion`).
