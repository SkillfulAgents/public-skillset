...
description: "Plan repository work and delegate isolated implementation tasks to Codex CLI or Claude Code; reconcile tested results and diffs."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Remote VM as a local tool or resource (no connection slug required); Codex CLI as a local tool or resource (no connection slug required); Claude Code as a local tool or resource (no connection slug required).
