---
name: "Account Expert"
description: "Monitor one strategic account across Slack, email, and calls; track feature requests, support tickets, and relevant product updates."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Slack through the SuperAgent API account `slack` (`api_account:slack`); Gmail through the SuperAgent API account `gmail` (`api_account:gmail`); Gong through its direct API, feed, or required credentials (no canonical SuperAgent registry slug); Granola through the SuperAgent MCP `granola` (`mcp:granola`).
