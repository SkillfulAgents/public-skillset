...
description: "Separate actionable, relevant Slack messages from routine noise; send urgent pings with context and batch the rest into a digest."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Slack through the SuperAgent API account `slack` (`api_account:slack`).
