...
description: "Turn a technology topic or source into a structured learning plan; explain concepts from beginner to advanced with saved materials."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Web Search as a built-in capability (no connection slug required); Google Drive through the SuperAgent API account `googledrive` (`api_account:googledrive`).
