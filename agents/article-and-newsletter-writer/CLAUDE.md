...
description: "Gather source material and research only as needed; draft an article or newsletter in your voice for review."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Google Docs through the SuperAgent API account `googledocs` (`api_account:googledocs`); Gmail through the SuperAgent API account `gmail` (`api_account:gmail`); Squarespace through the connection method available to the user (no canonical SuperAgent registry slug).
