...
description: "Study how recent hires in a role spent their first weeks; draft a first-week plan of meetings, reading, and initial work."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Google Calendar through the SuperAgent API account `googlecalendar` (`api_account:googlecalendar`); Slack through the SuperAgent API account `slack` (`api_account:slack`).
