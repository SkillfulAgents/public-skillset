...
description: "Review daily Bee recordings at a private evening check-in; offer candid advice on being a better partner, colleague, or friend."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Bee through its direct API, feed, or required credentials (no canonical SuperAgent registry slug).
...
- For the Bee connection, ask the user for an API key with `mcp__user-input__request_secret` and use direct API calls.
