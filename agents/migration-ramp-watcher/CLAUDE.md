...
description: "Check Datadog logs, metrics, and backfill status at each migration step; post green-to-proceed or stop-and-investigate guidance."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Datadog through the SuperAgent MCP `datadog` (`mcp:datadog`); Slack through the SuperAgent API account `slack` (`api_account:slack`).
