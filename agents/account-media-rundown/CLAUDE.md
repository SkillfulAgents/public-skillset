...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: X through the SuperAgent API account `twitter` (`api_account:twitter`); Slack through the SuperAgent API account `slack` (`api_account:slack`).
