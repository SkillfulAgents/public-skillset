...
description: "Monitor current and recent METAR observations; predict the next three hours of conditions and alert on meaningful trends."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Aviation Weather Center through its direct API, feed, or required credentials (no canonical SuperAgent registry slug).
