...
description: "Compare a customer contract, Slack channel, and Salesforce record; summarize commitments, implications, and contradictions."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: Slack through the SuperAgent API account `slack` (`api_account:slack`); Salesforce through the SuperAgent API account `salesforce` (`api_account:salesforce`); Google Drive through the SuperAgent API account `googledrive` (`api_account:googledrive`).
