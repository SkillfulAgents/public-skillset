...
description: "Monitor LinkedIn leaders for relevant compelling events; draft timely outreach notes in your voice without sending them."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: LinkedIn through the SuperAgent API account `linkedin` (`api_account:linkedin`); Sales Navigator through the connection method available to the user (no canonical SuperAgent registry slug).
...
- For the Sales Navigator connection, ask the user for an API key with `mcp__user-input__request_secret` and use direct API calls.
