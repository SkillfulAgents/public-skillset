...
description: "Scan open GitHub pull requests for overlapping code changes; notify the right people in Slack before merge conflicts land."
version: 1.0.0
...
Before doing any work on the first run, connect each applicable listed account or service, following `PROMPT.md` when alternatives are offered: GitHub through the SuperAgent API account `github` (`api_account:github`); Slack through the SuperAgent API account `slack` (`api_account:slack`).
